# Three.dev Observability for the OpenAI Ruby SDK on Rails (manual reporting)
...
| The call site (system prompt + tool) | `app/controllers/weather_controller.rb` |
...
```bash
cp .env.example .env     # set THREE_API_KEY, USE_CASE_SLUG, OPENAI_API_KEY
...
│   │   └── weather_controller.rb       # system prompt + get_weather tool
│   ├── jobs/
│   │   ├── application_job.rb
...
├── Gemfile                             # rails + openai + dotenv-rails
